    ...
    <v-container class="mb-5">
      ...
      <v-row align="center" justify="center" v-if="clientAccount">
            <v-col cols="12" class="d-flex justify-space-between" >
                ...
                <v-btn color="info" depressed @click="addModal = !addModal">Add Document</v-btn>
            </v-col>
        </v-row>
      ...
      <v-card color="grey lighten-3" v-if="!loading && client.company" class="mb-5">
...
export default {
  ...
  created() {
    ...
    if(!this.getDocuments.length) {
      ...
      this.fetchDocuments()
        ...
        this.documents = this.getDocuments.filter(doc => !doc.isArchived && doc.client_id == this.client.id)
  ...
  watch: {
    getDocuments(val) {
      if(val) {
        ...
        this.loading = false
        this.documents = val.filter(doc => !doc.isArchived && doc.client_id == this.client.id)
        // Sorting in Ascending Order
        this.documents = this.documents.sort((a,b) => new Date(a.month) - new Date(b.month))
        this.json_data = this.documents
        this.addModal = false
  ...
  data() {
    return {
      ...
      headers: [
        { text: 'File Name', sortable: true, value: 'file_name', width: '60%'},
